...
const BalanceService = () => {
  return {
      ...
      await execute((client) =>
        client.balance
          ...
          .then(async (balance) => {
            ...
            currencies.forEach((currency) => (defaultSpend[currency] = 0));
            ...
            if (balance.ownerType === OwnerType.GROUP) {
                ...
                .then((details) => {
                  ...
                  details.forEach((detail) => {
                    const amount = Math.ceil(
                      detail.amount / detail.shouldPayList.length,
                    );
                    detail.shouldPayList.forEach((shouldPay) => {
                      const userId = shouldPay.userId;
                      if (!res[userId])
                        res[userId] = {
                          ...defaultSpend,
                        };
                      res[userId][detail.currency] += amount;
                    });
                  });
                  return res;
                });
              const paidDetail = await client.detail
                ...
                .then((details) => {
                  ...
                  details.forEach((detail) => {
                    res[detail.payById] = res[detail.payById] || {};
                    res[detail.payById][detail.currency] = detail._sum
                      .amount as number;
                  });
                  return res;
                });
            ...
            res.forEach(
              (detail) =>
                ((detail as unknown as DetailView).shouldPay =
                  detail.shouldPayList.map((info) => info.user)),
            );
    ...
    getDetailById: async (id: number) =>
      await execute((client) =>
        client.detail.findFirstOrThrow({
          where: { id },
          include: {
            payBy: true,
            shouldPayList: {
